...
export default function ShortlistDetailPage() {
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold text-slate-900">Shortlist creators</h1>
        ...
        {entries.map(({ entry, creator }) => (
          <div key={entry.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-900">{creator.displayName ?? creator.username}</p>
                <a href={creator.profileUrl} target="_blank" rel="noreferrer" className="text-xs text-slate-500 hover:text-indigo-600">
                  @{creator.username}
                </a>
              </div>
              <select
            ...
            <textarea
              ...
              rows={2}
              className="mt-3 w-full resize-none rounded-md border border-slate-300 bg-white px-3 py-2 text-xs text-slate-800"
            />
          </div>
        ))}
