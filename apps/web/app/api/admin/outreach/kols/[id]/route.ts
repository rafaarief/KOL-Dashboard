...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  if (status && status !== before.status) {
    const [event] = await db
      .insert(schema.kolOutreachEvents)
      .values({ kolOutreachId: params.id, eventType: "status_changed", fromStatus: before.status, toStatus: status, createdByUserId: session.user.id })
      .returning();
  ...
  if (logFollowUp) {
    const [event] = await db
      .insert(schema.kolOutreachEvents)
      .values({ kolOutreachId: params.id, eventType: "follow_up", note: notes || null, createdByUserId: session.user.id })
      .returning();
    newEvents.push(event);
  } else if (notes !== undefined && notes !== before.notes) {
    const [event] = await db
      .insert(schema.kolOutreachEvents)
      .values({ kolOutreachId: params.id, eventType: "note", note: notes || null, createdByUserId: session.user.id })
      .returning();
    newEvents.push(event);
  }
