    ...
    <div className="rounded-xl border border-slate-800 bg-slate-900 p-5">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <p className="font-medium text-slate-100">{creator.displayName ?? creator.username}</p>
            <p className="text-sm text-slate-400">@{creator.username}</p>
          </div>
        </div>
        <div className="text-right">
          <p className="text-lg font-semibold text-indigo-400">{Number(result.finalScore).toFixed(0)}%</p>
          <p className="text-xs text-slate-400">{result.rankingLabel}</p>
        </div>
      </div>

      <div className="mt-4 grid grid-cols-2 gap-x-6 gap-y-1 text-sm text-slate-300 sm:grid-cols-4">
        <div>
      ...
      <p className="mt-4 rounded-lg bg-slate-950 px-3 py-2 text-sm text-slate-300">{result.rankingExplanation}</p>
